...
describe("MRB — golden table", () => {
  // Rijksdeel @ 1500kg (Art. 23): 64.24 + 6×17.27 = 167.86/q.
  // Opcenten enters only on the rijksdeel; brandstoftoeslag is added after.
  const rd1500 = 167.86;
  const cases: Array<{
    name: string;
    weightKg: number;
    ...
    // Petrol/Overijssel @ 1500kg: rijksdeel × (1+0.822) × 4.
    { name: "Petrol 1500kg Overijssel 2026", weightKg: 1500, p: "petrol", province: "Overijssel", taxYear: 2026, expected: rd1500 * 1.822 * 4 },
    // Diesel @ 1500kg: + dieseltoeslag 153.00 + 6×16.57 = 252.42 (opcenten-free).
    { name: "Diesel 1500kg Overijssel 2026", weightKg: 1500, p: "diesel", province: "Overijssel", taxYear: 2026, expected: (rd1500 * 1.822 + 252.42) * 4 },
    // Diesel + fijnstoftoeslag: +19% over (rijksdeel + dieseltoeslag).
    { name: "Diesel 1500kg + fijnstof", weightKg: 1500, p: "diesel", province: "Overijssel", taxYear: 2026, dieselFijnstof: true, expected: (rd1500 * 1.822 + 252.42 + (rd1500 + 252.42) * 0.19) * 4 },
    // LPG with G3 installation (default): toeslag 19.05 + 6×19.05 = 133.35.
    { name: "LPG-G3 1500kg Overijssel 2026", weightKg: 1500, p: "lpg", province: "Overijssel", taxYear: 2026, expected: (rd1500 * 1.822 + 133.35) * 4 },
    // LPG without G3: toeslag 172.08 + 6×18.22 = 281.40.
    { name: "LPG (non-G3) 1500kg Overijssel 2026", weightKg: 1500, p: "lpg", province: "Overijssel", taxYear: 2026, lpgG3: false, expected: (rd1500 * 1.822 + 281.40) * 4 },
    // EV/Overijssel @ 1500kg in 2026: full rate × (1−0.30) korting (art. 23b → pay 70%).
    { name: "EV 1500kg Overijssel 2026", weightKg: 1500, p: "ev", province: "Overijssel", taxYear: 2026, expected: rd1500 * 1.822 * 4 * 0.7 },
    // EV/Overijssel @ 1500kg in 2030: 0% korting.
    { name: "EV 1500kg Overijssel 2030", weightKg: 1500, p: "ev", province: "Overijssel", taxYear: 2030, expected: rd1500 * 1.822 * 4 * 1.0 },
    // Different province: Zuid-Holland has highest opcenten (1.044); Groningen 0.957.
    { name: "Petrol 1500kg Groningen", weightKg: 1500, p: "petrol", province: "Groningen", taxYear: 2026, expected: rd1500 * 1.957 * 4 },
    { name: "Petrol 1500kg Noord-Holland (lowest)", weightKg: 1500, p: "petrol", province: "Noord-Holland", taxYear: 2026, expected: rd1500 * 1.821 * 4 },
    // Light car: rijksdeel 29.12/q for 600kg.
    { name: "Petrol 600kg Overijssel", weightKg: 600, p: "petrol", province: "Overijssel", taxYear: 2026, expected: 29.12 * 1.822 * 4 },
    // Heavy car: rijksdeel 64.24 + 21×17.27 = 426.91/q for 3000kg.
    { name: "Petrol 3000kg Overijssel", weightKg: 3000, p: "petrol", province: "Overijssel", taxYear: 2026, expected: 426.91 * 1.822 * 4 },
  ];
